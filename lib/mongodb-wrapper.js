...
var Database = exports.Database = function (host, port, name, prefix, username, password) {
	...
	var reset = function() {
        ...
        // we should allow for many listeners for open if we want
	...
	var connectionOpened = function(err, connection) {
	...
	var notifyListeners = function(err) {
...
var Connection = exports.Connection = function (host, port, name) {
	...
	if (Array.isArray(host)) {
		...
		server = new mongodb.Server(host, port, { native_parser: false });
	}

	Connection.__super__.constructor.call(this, name, server, { w: 1 });
	this.reused = 0;
	this.started = 0;
	this.completed = 0;
